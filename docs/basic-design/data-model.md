...
| 論点 | 決定 | 根拠 |
| --- | --- | --- |
| エンティティ | `Account`, `Project`, `ProjectPeriodHistory`, `WbsTask`, `WbsTaskPlanHistory`, `WbsTaskProgressHistory`, `StudyLog`, `AiPlanGenerationRequest`, `AiPlanSource`, `AiGenerationJob`, `AiPlanDraft` を採用する | 15章の主要データを過不足なく表現できる。AI関連はMVP3に分離する |
...
AI学習計画生成の入力を表す。MVP3で作成する。

| 項目 | 方針 |
| --- | --- |
| テーブル名 | `ai_plan_generation_requests` |
| 主なカラム | `id`, `account_id`, `source_type`, `learning_goal`, `start_date`, `target_end_date`, `constraints_json`, `created_at`, `updated_at` |
| 主キー | `id` |
| 外部キー | `account_id` -> `accounts.id` |
| MVP | MVP3 |

`constraints_json` には平日・土日それぞれの学習可能時間、学習できない曜日、日程補足、重点範囲、軽く確認する範囲、除外範囲、ユーザー入力の数量条件、WBS分割単位を保持する。WBS分割単位は `SECTION`、`PAGE`、`QUESTION_SET`、`AI` の列挙値とする。概要、教材名、直接入力目次、修正済みOCR結果は重複保持せず、複数教材・複数画像を表現できる `ai_plan_sources` に保存する。教材画像ファイルは永続保存しない。

### 4.10 AiPlanSource

WBS下書き生成に利用した入力元を表す。概要、直接入力目次、OCR画像ごとの修正済みテキストを区別する。
...
画像そのものは保存せず、OCR後にユーザーが修正したテキストだけを入力元として保持する。`temporary_key` はOpenAIのWBS生成結果と入力元を対応付けるために使用し、業務DBのUUIDは送信しない。`source_order` は複数教材・複数画像の順序を維持する。

### 4.11 AiGenerationJob
...
| `ai_plan_sources` | AiPlanSource | MVP3 | WBS生成に利用した概要・目次・修正済みOCRテキストを保持 |
| `ai_generation_jobs` | AiGenerationJob | MVP3 | WBS生成の状態、期限、版、利用量を保持 |
...
| 10.10 AI学習計画生成 | 入力、入力元、非同期ジョブ、WBS下書きをMVP3の独立データとして追加 |
...
| AI下書きの正規化 | MVP3では `draft_wbs_tasks_json` に保持し、各LEAFに対応する入力元の`sourceTemporaryKeys`を含める。長期保存や詳細な差分監査が必要になった場合だけ下書きタスクの正規化を再検討する |
